...
const registerSchema = Joi.object({
    ...
    roll_number: Joi.string().alphanum().required().messages({
    ...
    batch: Joi.string()
        ...
        .messages({
            ...
            'string.pattern.base':
                'Batch must be exactly 4 digits (e.g., 2025)',
...
const modifySemesterSchema = Joi.object({
    new_semester: Joi.number().integer().min(1).max(10).required().messages({
        'number.base': 'Semester must be a number',
        'number.min': 'Semester must be between 1 and 10',
        'number.max': 'Semester must be between 1 and 10',
...
const markAttendanceSchema = Joi.object({
    ...
    lecture_date: Joi.date().iso().required().messages({
    ...
    status: Joi.string()
        ...
        .messages({
            'any.only':
                'Status must be one of: present, absent, medical, cancelled',
...
const updateAttendanceStatusSchema = Joi.object({
    log_id: Joi.string().required().messages({
